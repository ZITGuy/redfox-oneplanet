...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['MessageTemplate.name', '<?php __('Name'); ?>']
,			['MessageTemplate.body', '<?php __('Body'); ?>']
,			['MessageTemplate.default_body', '<?php __('Default Body'); ?>']
,			['MessageTemplate.placeholders', '<?php __('Placeholders'); ?>']
,			['MessageTemplate.created', '<?php __('Created'); ?>']
,			['MessageTemplate.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var messageTemplateSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'messageTemplates', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					messageTemplateSearchWindow.close();
					var conditions = finalGrouping(ParamStore.getRange());
					store_messageTemplates.reload({
						  params: {
								start: 0,
								limit: list_size,
...
var messageTemplateSearchWindow = new Ext.Window({
	title: '<?php __('Search MessageTemplate'); ?>',
	modal: true,
	items: messageTemplateSearchForm,
	resizable: false,
	width: 550,
	height: 400,

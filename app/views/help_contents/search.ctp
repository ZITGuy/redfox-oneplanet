...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['HelpContent.name', '<?php __('Name'); ?>']
,			['HelpContent.code', '<?php __('Code'); ?>']
,			['HelpContent.content', '<?php __('Content'); ?>']
,			['HelpContent.created', '<?php __('Created'); ?>']
,			['HelpContent.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var helpContentSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'helpContents', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					helpContentSearchWindow.close();
					var conditions = finalGrouping(ParamStore.getRange());
					store_helpContents.reload({
						  params: {
								start: 0,
								limit: list_size,
...
var helpContentSearchWindow = new Ext.Window({
	title: '<?php __('Search HelpContent'); ?>',
	modal: true,
	items: helpContentSearchForm,
	resizable: false,
	width: 550,
	height: 400,

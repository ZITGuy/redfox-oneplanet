...
class EduRegistrationsController extends EduAppController {
	...
	function print_achievement_report($id) {
		...
		foreach($rqrs as &$rqr) {
			...
			if($rqr['EduCourse']['edu_class_id'] != $registration['EduRegistration']['edu_class_id']) {
				$rqr['EduSubject'] = null;
				continue;
			}
        ...
        //$this->log($registration, 'card_printing_achievement_report');
         ...
         $exemption_records = $this->EduExemption->find('all', array('conditions' => array(
        	    'edu_student_id' =>  $registration['EduRegistration']['edu_student_id'])));
        	$exemptions = array();
		foreach($exemption_records as $ar){
			$exemptions[] = $ar['EduExemption']['edu_course_id'];
		}
		
		$this->set('registration', $registration);
		$this->set('evaluation_values', $evaluation_values);
		$this->set('scales', $scales);
		$this->set('exemtions', $exemptions);
	}
    ...
    function list_data_report($id = null) {
	   ...
	   $section = $this->EduSection->read(null, $edu_section_id);
        ...
        if($section['EduSection']['edu_academic_year_id'] == $eduAcademicYearId)
    ...
    private function deleteRegistration($id, $deleteEverything = 'false') {
        if ($deleteEverything == 'true') {
            ...
            foreach ($regs as $r) {
                $this->EduRegistration->delete($r['EduRegistration']['id']);
            }
            return true;
        } else {
            $this->EduRegistration->id = $id;
            $this->EduRegistration->read(null, $id);
            $this->EduRegistration->set('deleted', true);
            if ($this->EduRegistration->save()) {
                $this->log('Reg with id ' . $id . ' deleted by ' . $this->Session->read('Auth.User.username'), 'reg_del');
                return true;
            } else {
                $this->log(pr($this->EduRegistration->validationErrors, true), 'reg_del');
            }
            return false;
        }
    }
    ...
    function rpt_student_parent_data() {
        $this->loadModel('Edu.EduAcademicYear');
    ...
    function rpt_view_student_parent_data($id = null, $title = null) {
        $this->layout = 'ajax';
        ...
        $students = $this->EduStudent->query("SELECT st.name, st.nationality, fpd.first_name, fpd.middle_name, fpd.last_name, fpd.nationality, mpd.first_name, mpd.middle_name, mpd.last_name, mpd.nationality, fpd.mobile, mpd.mobile, fpd.occupation, mpd.occupation, fpd.email, mpd.email" .
            " FROM `edu_students` st " .
            " INNER JOIN `edu_parents` pa ON st.edu_parent_id = pa.id" .
            " INNER JOIN `edu_parent_details` fpd ON pa.id = fpd.edu_parent_id AND fpd.relationship = 'father'" .
            " INNER JOIN `edu_parent_details` mpd ON pa.id = mpd.edu_parent_id AND mpd.relationship = 'mother'" .
            " WHERE st.status_id = 1 AND st.deleted = 0 ORDER BY st.name ASC");

        $this->set('edu_students', $students);

        $this->log(print_r($students, true), 'debug_log');

...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EduTrainingCategory.name', '<?php __('Name'); ?>']
,			['EduTrainingCategory.deleted', '<?php __('Deleted'); ?>']
,			['EduTrainingCategory.created', '<?php __('Created'); ?>']
,			['EduTrainingCategory.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var eduTrainingCategorySearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduTrainingCategorySearchWindow.close();
...
var eduTrainingCategorySearchWindow = new Ext.Window({
	title: '<?php __('Search EduTrainingCategory'); ?>',
	modal: true,
	items: eduTrainingCategorySearchForm,
	resizable: false,
	width: 550,
	height: 400,

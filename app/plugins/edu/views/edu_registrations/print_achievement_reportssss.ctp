...
$html .= '<table width=100%><tr><td width=25% valign=bottom><img height=100px src="http://'. Configure::read('domain') . Configure::read('localhost_string') . '/img/opis_header.jpg"> <br/> </td>';
$html .= '</tr></table>';
...
$html .= '
	...
	foreach($course['Rqrs'] as $rqr) {
		...
		$cgpas[$ttcount] += (isset($scale_values[$scale_result])? $scale_values[$scale_result]: 0); 
		$html .= '	
					<td rowspan=3' . ($registration['EduClass']['cvalue'] > 4? ' style="margin:0; padding:0;"': '') . '>
						<table border=1' . ($registration['EduClass']['cvalue'] > 4? ' style="margin:0;"': '') . '><tr><td> ' . $scale_result . ' </td></tr></table><br/>
						<p>' . $num_result . '</p>
					</td>';
		$ttcount++;
	}
...
foreach($registration['EvaluationCategory'] as $category) {
	...
	foreach($category['Evaluations'] as $evaluation) {
		...
		elseif($category['EduEvaluationCategory']['name'] == 'Additional Work and Study Skills' && 
  			$registration['EduClass']['cvalue'] > 4)
			$html .=  ($evcount == 1? '<td rowspan=2>T1</td>': ($evcount == 3? '<td rowspan=2>T2</td>': ($evcount == 5? '<td rowspan=1>T3</td>': ''))) . 	
				'</tr>';
		else 
...
$html .= '<table width=100% border=1>
			...
			foreach($registration['EduRegistrationQuarter'] as $rq) {
				$tgpa = '0.00';
				if($course_count-2 > 0)  {
					$tgpa = $cgpas[$cgpacount] / ($course_count-2);

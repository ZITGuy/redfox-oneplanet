...
,			['EduAssignment.start_date', '<?php __('Start Date'); ?>']
,			['EduAssignment.end_date', '<?php __('End Date'); ?>']
,			['EduAssignment.created', '<?php __('Created'); ?>']
,			['EduAssignment.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var eduAssignmentSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduAssignmentSearchWindow.close();
...
var eduAssignmentSearchWindow = new Ext.Window({
	title: '<?php __('Search EduAssignment'); ?>',
	modal: true,
	items: eduAssignmentSearchForm,
	resizable: false,
	width: 550,
	height: 400,

...
function AddParameter(field, value){
	...
	if(currentlyToggled == "IN"){
		...
		row = [[field, 'array("Task.name BETWEEN ? AND ?" => array( ' + InsertQuotations(value) + ' ))']];
	}
...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['Task.name', '<?php __('Name'); ?>'],
			['Task.controller', '<?php __('Controller'); ?>'],
			['Task.action', '<?php __('Action'); ?>'],
			['Task.parameter', '<?php __('Parameter'); ?>'],
			['Task.list_order', '<?php __('List Order'); ?>'],
			['Task.built_in', '<?php __('Is Builtin'); ?>'],			
			['Task.created', '<?php __('Created'); ?>'],			
			['Task.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var taskSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'tasks', 'action' => 'search')); ?>',
...
var taskSearchWindow = new Ext.Window({
	title: '<?php __('Search Task'); ?>',
	modal: true,
	items: taskSearchForm,
	resizable: false,
	width: 550,
	height: 400,

//<script>
    ...
    function ViewParentEduCourses(id) {
        Ext.Ajax.request({
            url: "<?php echo $this->Html->url(array('controller' => 'edu_courses', 'action' => 'index_subject_m')); ?>/" + id,
            success: function(response, opts) {
    ...
    if (center_panel.find('id', 'eduSubject-tab-m') != "") {
        var p = center_panel.findById('eduSubject-tab-m');
        ...
        var p = center_panel.add({
            ...
            listeners: {
                celldblclick: function() {
                    ViewEduSubject(Ext.getCmp('eduSubject-tab-m').getSelectionModel().getSelected().data.id);
                }
            },
            ...
            tbar: new Ext.Toolbar({
                items: [{
                        ...
                        xtype: 'tbbutton',
                        text: '<?php __('Maintain Courses'); ?>',
                        icon: 'img/table_view.png',
                        id: 'maintain-eduCourse-m',
                        disabled: true,
                        cls: 'x-btn-text-icon',
                        handler: function(btn) {
                            var sm = p.getSelectionModel();
                        ...
                        id: 'eduSubject_search_field-m',
                        listeners: {
                            specialkey: function(field, e) {
                                if (e.getKey() == e.ENTER) {
                                    SearchByEduSubjectName(Ext.getCmp('eduSubject_search_field-m').getValue());
                                }
                            }
                        }
                    }, {
        ...
        p.getSelectionModel().on('rowselect', function(sm, rowIdx, r) {
            p.getTopToolbar().findById('edit-eduSubject-m').enable();
            p.getTopToolbar().findById('delete-eduSubject-m').enable();
            p.getTopToolbar().findById('view-eduSubject-m').enable();
            p.getTopToolbar().findById('maintain-eduCourse-m').enable();
            if (this.getSelections().length > 1) {
                p.getTopToolbar().findById('edit-eduSubject-m').disable();
                p.getTopToolbar().findById('view-eduSubject-m').disable();
                p.getTopToolbar().findById('maintain-eduCourse-m').disable();
            }
        });
        p.getSelectionModel().on('rowdeselect', function(sm, rowIdx, r) {
            if (this.getSelections().length > 1) {
                ...
                p.getTopToolbar().findById('edit-eduSubject-m').disable();
                p.getTopToolbar().findById('view-eduSubject-m').disable();
                p.getTopToolbar().findById('delete-eduSubject-m').disable();
                p.getTopToolbar().findById('maintain-eduCourse-m').disable();
            }
        });
        center_panel.setActiveTab(p);

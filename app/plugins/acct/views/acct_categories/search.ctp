...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['AcctCategory.name', '<?php __('Name'); ?>'],			
			['AcctCategory.prefix', '<?php __('Prefix'); ?>'],			
			['AcctCategory.code', '<?php __('Code'); ?>'],			
			['AcctCategory.postfix', '<?php __('Postfix'); ?>'],			
			['AcctCategory.last_code', '<?php __('Last Code'); ?>'],			
			['AcctCategory.created', '<?php __('Created'); ?>'],			
			['AcctCategory.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var acctCategorySearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'acct_categories', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					acctCategorySearchWindow.close();
...
var acctCategorySearchWindow = new Ext.Window({
	title: '<?php __('Search Account Category'); ?>',
	modal: true,
	items: acctCategorySearchForm,
	resizable: false,
	width: 550,
	height: 400,

...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EduClassLevel.name', '<?php __('Name'); ?>']
		]
	}),
	mode: 'local',
...
var eduClassLevelSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'eduClassLevels', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduClassLevelSearchWindow.close();
...
var eduClassLevelSearchWindow = new Ext.Window({
	title: '<?php __('Search EduClassLevel'); ?>',
	modal: true,
	items: eduClassLevelSearchForm,
	resizable: false,
	width: 550,
	height: 400,

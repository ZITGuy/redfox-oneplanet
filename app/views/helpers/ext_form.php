...
class ExtFormHelper extends AppHelper {
    ...
    function input($field, $options = array(), $returnString = false) {
        ...
        foreach ($configOptions as $property => $value) {
            $out .= "\t\t\t\t\t" . $property . ": " . ((in_array($value, array('true', 'false', '{iconCls: \'upload-icon\'}')) || (strpos($value, '0') <> 0 && is_numeric($value)) || (is_numeric($value)) || ($property == 'listeners') || ($property == 'disabledDays') || ($property == 'disabledDates') || ($property == 'buttonCfg') || ($property == 'maskRe') || ($property == 'plugins') || ($property == 'minValue') || ($property == 'maxValue') || ($property == 'validator')) ? $value : "'" . $value . "'");

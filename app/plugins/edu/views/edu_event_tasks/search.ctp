...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EduCalendarEvent.name', '<?php __('EduCalendarEvent'); ?>']
,			['EduEventTask.task', '<?php __('Task'); ?>']
,			['EduEventTask.permissions', '<?php __('Permissions'); ?>']
,			['EduEventTask.created', '<?php __('Created'); ?>']
,			['EduEventTask.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var eduEventTaskSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduEventTaskSearchWindow.close();
...
var eduEventTaskSearchWindow = new Ext.Window({
	title: '<?php __('Search EduEventTask'); ?>',
	modal: true,
	items: eduEventTaskSearchForm,
	resizable: false,
	width: 550,
	height: 400,

...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['Authorization.name', '<?php __('Name'); ?>']
,			['Authorization.command_script', '<?php __('Command Script'); ?>']
,			['Maker.name', '<?php __('Maker'); ?>']
,			['Authorizer.name', '<?php __('Authorizer'); ?>']
,			['Authorization.status', '<?php __('Status'); ?>']
,			['Authorization.created', '<?php __('Created'); ?>']
,			['Authorization.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var authorizationSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'authorizations', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					authorizationSearchWindow.close();
...
var authorizationSearchWindow = new Ext.Window({
	title: '<?php __('Search Authorization'); ?>',
	modal: true,
	items: authorizationSearchForm,
	resizable: false,
	width: 550,
	height: 400,

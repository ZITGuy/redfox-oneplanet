...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EduClass.name', '<?php __('Class'); ?>'],
			['EduSubject.name', '<?php __('Subject'); ?>'],
			['EduCourse.description', '<?php __('Description'); ?>'],
			['EduCourse.created', '<?php __('Created'); ?>'],
			['EduCourse.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var eduCourseSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
...
var eduCourseSearchWindow = new Ext.Window({
	title: '<?php __('Search Course'); ?>',
	modal: true,
	items: eduCourseSearchForm,
	resizable: false,
	width: 550,
	height: 400,

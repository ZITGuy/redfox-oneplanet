...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['Classification.name', '<?php __('Name'); ?>']
,			['ParentClassification.name', '<?php __('ParentClassification'); ?>']
,			['Classification.lft', '<?php __('Lft'); ?>']
,			['Classification.rght', '<?php __('Rght'); ?>']
		]
	}),
	mode: 'local',
...
var classificationSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'classifications', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					classificationSearchWindow.close();
					var conditions = finalGrouping(ParamStore.getRange());
					store_classifications.reload({
						  params: {
								start: 0,
								limit: list_size,
...
var classificationSearchWindow = new Ext.Window({
	title: '<?php __('Search Classification'); ?>',
	modal: true,
	items: classificationSearchForm,
	resizable: false,
	width: 550,
	height: 400,

...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EduCampus.name', '<?php __('Name'); ?>']
,			['EduCampus.address', '<?php __('Address'); ?>']
,			['EduCampus.created', '<?php __('Created'); ?>']
,			['EduCampus.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var eduCampusSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'eduCampuses', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduCampusSearchWindow.close();
					var conditions = finalGrouping(ParamStore.getRange());
					store_eduCampuses.reload({
						  params: {
								start: 0,
								limit: list_size,
...
var eduCampusSearchWindow = new Ext.Window({
	title: '<?php __('Search EduCampus'); ?>',
	modal: true,
	items: eduCampusSearchForm,
	resizable: false,
	width: 550,
	height: 400,

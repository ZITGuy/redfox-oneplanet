...
$html = '<style>
    body { font-family: DejaVuSansCondensed; font-size: 10pt;  }
...
if($section['EduClass']['grading_type'] == 'G'){
    if($mode == 'inner'){
        $url = 'http://' . Configure::read('domain') . Configure::read('localhost_string') . '/edu/edu_registrations/print_cards_for_preschool_inner/' .  $edu_section_id . (isset($edu_registration_id)? '/' . $edu_registration_id: '');
        $content = file_get_contents($url);
        $html .= substr($content, 3);

    } elseif($mode == 'outer'){

    } else {
        // both sides in a PDF document

    }
} else {
    if($mode == 'inner'){
        $url = 'http://' . Configure::read('domain') . Configure::read('localhost_string') . '/edu/edu_registrations/print_cards_for_numeric_inner/' .  $edu_section_id . (isset($edu_registration_id)? '/' . $edu_registration_id: '');
        $content = file_get_contents($url);
        $html .= substr($content, 3);
    } elseif($mode == 'outer'){
...
if($grading_type == 'G'){
    ...
    $mpdf=new mPDF('c','A4-L','','Nyala',10,10,10,10,19,14); 
}

//$mpdf->SetWatermarkText('Sample Report Card');
//$mpdf->watermark_font = 'DejaVuSansCondensed';
//$mpdf->showWatermarkText = true;

$mpdf->SetWatermarkImage('http://' . Configure::read('domain') . Configure::read('localhost_string') . '/img/sample_card_bg3.png', 0.15, 'F');

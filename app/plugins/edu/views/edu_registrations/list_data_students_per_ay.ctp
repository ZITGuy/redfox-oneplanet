...
<?php $st = false; foreach($edu_registrations as $edu_registration){ if($st) echo ","; ?>			{
				...
				"edu_section":"<?php echo ($edu_registration['EduRegistration']['edu_section_id'] == 0? 'Not Sectioned Yet': $edu_registration['EduSection']['name']) . ' - ' . $edu_registration['EduCampus']['name']; ?>",
				"status":"<?php echo $edu_registration['Status']['name']; ?>",
				"registration_date":"<?php echo date('F d, Y', strtotime($edu_registration['EduRegistration']['created'])); ?>",

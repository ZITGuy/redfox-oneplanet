...
$html .= '
	...
	$html .= '
				<tr id="course_row">
					<td rowspan=3 colspan=2 style="margin:0; padding:0;"><table width=100% style="margin:0;">' . $items . '</table>' . '<br/>&nbsp;' . '</td>';
	foreach($course['Rqrs'] as $rqr) {
			...
			foreach($registration['EduRegistrationEvaluation'] as $rev) {
				if($rev['edu_quarter_id'] == $rq['edu_quarter_id'] && 
					$evaluation['EduEvaluation']['id'] == $rev['edu_evaluation_id']) {
						$q = $quarters[ $rq['edu_quarter_id']];
						
						$ev = (isset($evaluation_values[$rev['edu_evaluation_value_id']])  && ($q['EduQuarter']['status_id'] == 1 || $q['EduQuarter']['status_id'] == 8))? $evaluation_values[$rev['edu_evaluation_value_id']]: '-';
						break;
				}
			}
...
echo $html . '<br/><br/>
	<script>
	function myFunction() {
		document.getElementById("course_row").style.pageBreakInside = "avoid";
	}
	</script>
';

...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EduExtraPaymentSetting.name', '<?php __('EduExtraPaymentSetting'); ?>']
,			['EduStudent.name', '<?php __('EduStudent'); ?>']
,			['EduExtraPayment.is_paid', '<?php __('Is Paid'); ?>']
,			['EduExtraPayment.date_paid', '<?php __('Date Paid'); ?>']
,			['EduExtraPayment.paid_amount', '<?php __('Paid Amount'); ?>']
,			['EduExtraPayment.cheque_number', '<?php __('Cheque Number'); ?>']
,			['EduExtraPayment.cheque_amount', '<?php __('Cheque Amount'); ?>']
,			['EduExtraPayment.invoice', '<?php __('Invoice'); ?>']
,			['EduExtraPayment.transaction_ref', '<?php __('Transaction Ref'); ?>']
,			['EduExtraPayment.created', '<?php __('Created'); ?>']
,			['EduExtraPayment.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var eduExtraPaymentSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduExtraPaymentSearchWindow.close();
...
var eduExtraPaymentSearchWindow = new Ext.Window({
	title: '<?php __('Search EduExtraPayment'); ?>',
	modal: true,
	items: eduExtraPaymentSearchForm,
	resizable: false,
	width: 550,
	height: 400,

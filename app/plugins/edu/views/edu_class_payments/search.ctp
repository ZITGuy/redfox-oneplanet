...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EduClass.name', '<?php __('EduClass'); ?>']
,			['EduAcademicYear.name', '<?php __('EduAcademicYear'); ?>']
,			['EduClassPayment.enrollment_fee', '<?php __('Enrollment Fee'); ?>']
,			['EduClassPayment.registration_fee', '<?php __('Registration Fee'); ?>']
,			['EduClassPayment.created', '<?php __('Created'); ?>']
,			['EduClassPayment.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var eduClassPaymentSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'eduClassPayments', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduClassPaymentSearchWindow.close();
					var conditions = finalGrouping(ParamStore.getRange());
					store_eduClassPayments.reload({
						  params: {
								start: 0,
								limit: list_size,
...
var eduClassPaymentSearchWindow = new Ext.Window({
	title: '<?php __('Search EduClassPayment'); ?>',
	modal: true,
	items: eduClassPaymentSearchForm,
	resizable: false,
	width: 550,
	height: 400,

...
class EduParentsController extends EduAppController {
    ...
    var $api_url = 'http://portal-api.onenessacademy.edu.et/api'; // production server. Due to TLS issues, using http instead of https
    ...
    function push_parent_data_to_portal($id = null) {
        ...
        $this->log('Pushing parent data to portal for parent id: ' . $id, 'debug_push');
        $this->log('API URL: ' . $this->api_url . '/auth/login', 'debug_push');
        ...
        // why is the $response null?
        if($response === false) {
            $this->set('response', 'Curl error: ' . curl_error($curl));
            $this->log('Curl error: ' . curl_error($curl), 'debug_push');
            return;
        }
        ...
        */
        if($response['statusCode'] != 200) {
            $this->set('response', 'Error logging in to portal: ' . $response['message']);
            $this->log('Error logging in to portal: ' . $response['message'], 'debug_push');
            return;
        }

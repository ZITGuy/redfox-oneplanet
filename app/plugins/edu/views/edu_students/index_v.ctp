//<script>
        ...
        Ext.Ajax.request({
            ...
            failure: function(response, opts) {
                Ext.Msg.alert("<?php __('Error'); ?>",
                    "<?php __('Cannot get the Student view form. Error code'); ?>: " + response.status);
            }
        });
    }
    ...
    function SearchEduStudent() {
        Ext.Ajax.request({
            ...
            failure: function(response, opts) {
                Ext.Msg.alert("<?php __('Error'); ?>",
                    "<?php __('Cannot get the Student search form. Error Code'); ?>: " + response.status);
            }
        });
    }
	
	function  ViewEduStudentPDF(id){
		Ext.Ajax.request({
            url: "<?php echo $this->Html->url(array('controller' => 'edu_students', 'action' => 'view')); ?>/" + id,
            success: function(response, opts) {
                var eduStudent_data = response.responseText;
    ...
    function SearchByEduStudentName(value) {
        var conditions = '\'OR\' => array(\'EduStudent.name LIKE\' => \'%' + value +
            '%\', \'EduStudent.identity_number LIKE\' => \'%' + value + '%\')';
        ...
        var p = center_panel.add({
            ...
            tbar: new Ext.Toolbar({
                items: [{
                        ...
                        id: 'view-eduStudent',
                        tooltip: "<?php __('<b>View Student Profile</b><br />Click here to see details'); ?>",
                        ...
                        xtype: 'tbbutton',
                        text: "<?php __('Enrollment Cert'); ?>",
                        id: 'print-enrollment-cert',
                        tooltip: "<?php __('<b>View Enrollment Certificate</b><br />Click here to see details'); ?>",
                        icon: 'img/table_view.png',
                        cls: 'x-btn-text-icon',
                        disabled: true,
                        handler: function(btn) {
...
<?php $st = false;
    foreach ($statuses as $k => $v) {
		if ($st) { echo ","; } ?>['<?php echo $k; ?>', '<?php echo $v; ?>']<?php $st = true;
} ?>
                            ]
                        }),
                        displayField: 'name',
                        ...
                        cls: 'x-btn-text-icon',
                        text: "<?php __('...'); ?>",
                        tooltip: "<?php __('<b>Advanced Search...</b>'); ?>",
                        handler: function() {
                            SearchEduStudent();
                        }
                    }
        ...
        p.getSelectionModel().on('rowselect', function(sm, rowIdx, r) {
            ...
            p.getTopToolbar().findById('print-enrollment-cert').enable();
        });
        p.getSelectionModel().on('rowdeselect', function(sm, rowIdx, r) {
            if (this.getSelections().length > 0) {
                ...
                p.getTopToolbar().findById('print-enrollment-cert').disable();
            }
        });
        center_panel.setActiveTab(p);
    ...
    function popUpWindowEnrollmentCertificate(URLStr, left, top, width, height) {
        ...
        popUpWin_EnrollmentCertificate = open(URLStr, 'popUpWin',
    ...
    function printEnrollmentCertificate(id) {
        url = "<?php echo $this->Html->url(array(
            'controller' => 'edu_students', 'action' => 'enrollment_certificate', 'plugin' => 'edu')); ?>/" + id;

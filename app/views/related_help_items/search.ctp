...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['HelpItem.name', '<?php __('HelpItem'); ?>']
,			['RelatedHelpItem.name', '<?php __('RelatedHelpItem'); ?>']
		]
	}),
	mode: 'local',
...
var relatedHelpItemSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'relatedHelpItems', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					relatedHelpItemSearchWindow.close();
					var conditions = finalGrouping(ParamStore.getRange());
					store_relatedHelpItems.reload({
						  params: {
								start: 0,
								limit: list_size,
...
var relatedHelpItemSearchWindow = new Ext.Window({
	title: '<?php __('Search RelatedHelpItem'); ?>',
	modal: true,
	items: relatedHelpItemSearchForm,
	resizable: false,
	width: 550,
	height: 400,

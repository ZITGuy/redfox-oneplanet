...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EduRegistration.name', '<?php __('Name'); ?>']
,			['EduStudent.name', '<?php __('EduStudent'); ?>']
,			['EduSection.name', '<?php __('EduSection'); ?>']
,			['EduRegistration.created', '<?php __('Created'); ?>']
,			['EduRegistration.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var eduRegistrationSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduRegistrationSearchWindow.close();
...
var eduRegistrationSearchWindow = new Ext.Window({
	title: '<?php __('Search EduRegistration'); ?>',
	modal: true,
	items: eduRegistrationSearchForm,
	resizable: false,
	width: 550,
	height: 400,

...
var store_attendanceRecords = new Ext.data.GroupingStore({
	reader: new Ext.data.JsonReader({
		...
		fields: [
			'id', 'user', 'section', 'quarter', 'status', 'date', 'created'		
		]
	}),
	proxy: new Ext.data.HttpProxy({
		url: '<?php echo $this->Html->url(array('controller' => 'edu_attendance_records', 'action' => 'list_data')); ?>'
	}),	
...
function DailyAttendanceRecord(id) {
    Ext.Ajax.request({
        ...
        failure: function(response, opts) {
            Ext.Msg.alert('<?php __('Error'); ?>', '<?php __('Cannot get the attendance Record view form. Error code'); ?>: ' + response.status);
        }
    });
}

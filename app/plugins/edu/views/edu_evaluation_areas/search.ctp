...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EduEvaluationArea.name', '<?php __('Name'); ?>']
,			['EduEvaluationCategory.name', '<?php __('EduEvaluationCategory'); ?>']
,			['EduEvaluationArea.created', '<?php __('Created'); ?>']
,			['EduEvaluationArea.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var eduEvaluationAreaSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'eduEvaluationAreas', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduEvaluationAreaSearchWindow.close();
...
var eduEvaluationAreaSearchWindow = new Ext.Window({
	title: '<?php __('Search EduEvaluationArea'); ?>',
	modal: true,
	items: eduEvaluationAreaSearchForm,
	resizable: false,
	width: 550,
	height: 400,

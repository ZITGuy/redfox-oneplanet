...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['RestorePoint.name', '<?php __('Name'); ?>']
,			['RestorePoint.created', '<?php __('Created'); ?>']
,			['RestorePoint.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var restorePointSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'restorePoints', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					restorePointSearchWindow.close();
					var conditions = finalGrouping(ParamStore.getRange());
					store_restorePoints.reload({
						  params: {
								start: 0,
								limit: list_size,
...
var restorePointSearchWindow = new Ext.Window({
	title: '<?php __('Search RestorePoint'); ?>',
	modal: true,
	items: restorePointSearchForm,
	resizable: false,
	width: 550,
	height: 400,

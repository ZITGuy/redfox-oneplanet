//<script>
	...
	function viewRptRegisteredStudents() {
		...
		url = "<?php echo $this->Html->url(array('controller' => 'edu_registrations', 'action' => 'rpt_view_registered_students')); ?>/" + selected_academic_year_id + '/' + newstr;
		popUpWindow(url, 0, 0, 1200, 1200);
	}
	
	var RptRegisteredStudentsForm = new Ext.form.FormPanel({
		baseCls: 'x-plain',
		labelWidth: 100,
		labelAlign: 'right',
		url:'',
		...
		items: [
			<?php 
				$options = array('value' => 'Registered Students', 'id' => 'data[EduRegistration][report_title]');
	...
	var RptRegisteredStudentsWindow = new Ext.Window({
		title: '<?php __('Report: Registered Students'); ?>',
		width: 400,
		minWidth: 400,
		autoHeight: true,
		layout: 'fit',
		...
		buttonAlign:'right',
		items: RptRegisteredStudentsForm,
		tools: [{
			id: 'refresh',
			qtip: 'Reset',
			handler: function () {
				RptRegisteredStudentsForm.getForm().reset();
			},
			scope: this
		}, {
			id: 'help',
			...
			handler: function () {
				Ext.Msg.show({
					...
					msg: 'This form is used to set parameters to the Registered students report.',
			...
			handler: function () {
				if(RptRegisteredStudentsWindow.collapsed)
					RptRegisteredStudentsWindow.expand(true);
				else
					RptRegisteredStudentsWindow.collapse(true);
			}
		}],
		buttons: [  {
			text: '<?php __('Show report'); ?>',
			handler: function(btn){
				viewRptRegisteredStudents();
			}
		}, {
			text: '<?php __('Cancel'); ?>',
			handler: function(btn){
				RptRegisteredStudentsWindow.close();
			}
		}]
	});
	
	RptRegisteredStudentsWindow.show();

//<script>
    ...
    var EduStudentTransferForm = new Ext.form.FormPanel({
        baseCls: 'x-plain',
        labelWidth: 100,
        labelAlign: 'right',
        url:'<?php echo $this->Html->url(array('controller' => 'edu_students', 'action' => 'transfer_student')); ?>',
        ...
        items: [{
            ...
            ?>,
            <?php 
                $options_reason = array('fieldLabel' => 'Transfer Reason', 'xtype' => 'textarea');
                $this->ExtForm->input('reason', $options_reason);
            ?>
        ]
    });
		
    var EduStudentTransferWindow = new Ext.Window({
        title: '<?php __('Transfer Student(s)'); ?>',
        width: 700,
        minWidth: 700,
        autoHeight: true,
        layout: 'fit',
        ...
        buttonAlign:'right',
        items: EduStudentTransferForm,
        tools: [{
            id: 'refresh',
            qtip: 'Reset',
            handler: function () {
                EduStudentTransferForm.getForm().reset();
            },
            scope: this
        }, {
            id: 'help',
            ...
            handler: function () {
                if(EduStudentTransferWindow.collapsed)
                        EduStudentTransferWindow.expand(true);
                else
                        EduStudentTransferWindow.collapse(true);
            }
        }],
        buttons: [{
                text: '<?php __('Transfer'); ?>',
                handler: function(btn){
                    EduStudentTransferForm.getForm().submit({
                        ...
                        success: function(f,a){
                            ...
                            EduStudentTransferWindow.close();
                        },

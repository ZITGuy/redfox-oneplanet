//<script>
    ...
    function ChangeUserPassword(id) {
		Ext.Ajax.request({
            url: '<?php echo $this->Html->url(array('controller' => 'users', 'action' => 'change_password')); ?>/'+id,
            success: function(response, opts) {
                var user_data = response.responseText;
            ...
            failure: function(response, opts) {
                Ext.Msg.alert('<?php __('Error'); ?>', '<?php __('Cannot get the user password change form. Error code'); ?>: ' + response.status);
            }
		});
    }
    ...
    if(center_panel.find('id', 'user-tab') != "") {
		...
		var p = center_panel.add({
            ...
            columns: [
				...
				{header: "<?php __('Working Campus'); ?>", dataIndex: 'campus', sortable: true},
            ...
            tbar: new Ext.Toolbar({
                items: [{
                        ...
                        xtype: 'tbbutton',
                        text: '<?php __('Change Password'); ?>',
                        id: 'change-user-password',
                        tooltip:'<?php __('Change User Password'); ?>',
                        icon: 'img/table_edit.png',
                        cls: 'x-btn-text-icon',
                        disabled: true,
                        handler: function(btn) {
                            ...
                            var sel = sm.getSelected();
                            if (sm.hasSelection()){
                                ChangeUserPassword(sel.data.id);
                            }
                        }
                    }, ' ', '-', ' ', {
                        xtype: 'tbbutton',
	...
	p.getSelectionModel().on('rowselect', function(sm, rowIdx, r) {
		...
		p.getTopToolbar().findById('change-user-password').enable();
		...
		if(this.getSelections().length > 1){
			...
			p.getTopToolbar().findById('change-user-password').disable();

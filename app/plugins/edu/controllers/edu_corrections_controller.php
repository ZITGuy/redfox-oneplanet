...
class EduCorrectionsController extends EduAppController {
    ...
    public function add($id = null) {
        ...
        $this->EduAcademicYear->recursive = -1;
        $this->EduClass->recursive = -1;

        $edu_academic_years = $this->EduAcademicYear->find('list');
        $edu_classes = $this->EduClass->find('list');
        $this->set(compact('edu_academic_years', 'edu_classes'));
    }
    ...
    public function approve_correction($id = null) {
        ...
        if (!empty($this->data)) {
            ...
            if ($this->EduCorrection->save($this->data)) {
                ...
                // read all the registration results of the specific registration and take the averages from
                $rrs = $this->EduRegistrationResult->find('all', array(
                    'conditions' => array(
                        'EduRegistrationResult.edu_registration_id' => $correction['EduCorrection']['edu_registration_id']
                    )
                ));
                ...
                $cgpa = 0;
                foreach ($rrs as $r) {
                    $average += $r['EduRegistrationResult']['average'];
                    $cgpa += $this->getGPA($r['EduRegistrationResult']['scale_result']);
                }
                $average /= count($rrs);
                $cgpa /= count($rrs);

                // go for registration record correction on average and acgpa fields
                $this->EduRegistration->read(null, $correction['EduCorrection']['edu_registration_id']);
                $this->EduRegistration->set('average', $average);
                $this->EduRegistration->set('cgpa', $cgpa);

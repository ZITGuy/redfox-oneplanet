	...
	<tr>
		<td align="center">
			<center>
				<h1>' . $company_name . '</h1>
				<h2>' . $report_title . '</h2>
				<h3>Daily Transactions as of : ' . $date . '</h3>
			</center>
		</td>
	</tr>
	<tr>
...
foreach($transactions as $transaction) {
	$started = false;
    foreach($transaction['AcctJournal'] as $journal) {
		$html .= '
			<tr>
				<th>' . (!$started? $transaction['AcctTransaction']['name']: '') . '</th>
				<td>' . (!$started? $transaction['AcctTransaction']['description']: '') . '</td>
				<td>' . $journal['AcctAccount']['code'] . '</td>
				<td>' . $journal['AcctAccount']['name'] . '</td>
				<td align="right">' . $journal['dr'] . '</td>
				<td align="right">' . $journal['cr'] . '</td>
...
$mpdf->SetHeader('{DATE j-m-Y}||' . $report_title . ' - ' . $company_name);

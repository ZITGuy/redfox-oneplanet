...
<table border="0" width="100%">
	...
	<tr>
		<td align="right">
			<h2>Total Teachers: ' . (count($edu_teachers)) . '</h2>
			<h2>Campus: Main</h2>
		</td>
	</tr>
	<tr>
...
foreach($edu_sections as $sec) {
	if($sec['EduSection']['edu_teacher_id'] != 0) {
		$sections[$sec['EduSection']['edu_teacher_id']] = $sec;
	}
	if($sec['EduSection']['co_teacher_id'] != 0) {
		$sections[$sec['EduSection']['co_teacher_id']] = $sec;
	}
}
...
foreach($edu_teachers as $teacher) {
    $html .= '
		<tr>
			<th>' . $count++ . '</th>
			<th>' . $teacher['EduTeacher']['identity_number'] . '</th>
			<td>' . $teacher['User']['Person']['first_name'] . ' ' . $teacher['User']['Person']['middle_name'] . ' ' . ($teacher['User']['Person']['last_name'] == '[Last Name]'? '': $teacher['User']['Person']['last_name']) . '</td>
			<td>' . strtolower($teacher['User']['username']) . '</td>
			<td>' . $teacher['EduTeacher']['telephone_mobile'] . '</td>
			<td>' . strtolower($teacher['User']['email']) . '</td>
			<td>' . (isset($sections[$teacher['EduTeacher']['id']])? $sections[$teacher['EduTeacher']['id']]['EduClass']['name'] . ' - ' . $sections[$teacher['EduTeacher']['id']]['EduSection']['name']: '-') . '</td>
        </tr>';
}

...
$html .= '<table width=100% border=1>
			...
			<tr>
				...
				<td rowspan=2 valign=bottom>' . $registration['Homeroom']['User']['Person']['first_name'] . ' ' . $registration['Homeroom']['User']['Person']['middle_name'] . ' ' . $registration['Homeroom']['User']['Person']['last_name'] . '</td>
...
$html .= '  <tr>
				...
				<td>' . round($scale['EduScale']['min']) . '-' . round($scale['EduScale']['max'] - ($scale['EduScale']['max'] >= 100? 0: 1)) . '</td>
				<td>' . number_format($scale['EduScale']['grade_point'], 1, '.', ',') . '</td>
				<td>' . $scale['EduScale']['remark'] . '</td>
			</tr>';
		}
$html .= '</table>'; // . pr($registration['Rqrs'], true);
...
$html .= '
			<tr>
				<td bgcolor=#cccccc>' . $course_count . '</td>
				<td bgcolor=#cccccc>' . strtoupper($course['Subject']['name']) . ' / ' . $course['Subject']['name_am'] . '</td>';
			$course_count++;
			$tcount=1;
...
$items = (count($rqr['EduCourseItem']) == 0? '&nbsp;': ''); // $course['Course']['description']: '');
...
$html .= '
			<tr>
				<td rowspan=3 colspan=2 style="margin:0; padding:0;"><table width=100% style="margin:0;">' . $items . '</table></td>';
foreach($course['Rqrs'] as $rqr) {
$html .= '	
				<td rowspan=3 style="margin:0; padding:0;">
					<table border=1><tr><td> ' . ($rqr['EduRegistrationQuarterResult']['course_result'] == 0? '&nbsp;': str_replace('.00', '', $rqr['EduRegistrationQuarterResult']['course_result']) . '<sup>' . $rqr['EduRegistrationQuarterResult']['scale_result'] . '</sup>') . ' </td></tr></table>	
				</td>';
}
...
$html .= '		<td>
					T' . $ttcount . '<br/>' . (($rqr['EduRegistrationQuarterResult']['teacher_comment'] == '-')? '&nbsp;': $rqr['EduRegistrationQuarterResult']['teacher_comment']) . '
				</td>
			</tr>';
			$ttcount++;
}
...
$html .= '</table>'; // . pr($registration['Evaluation'], true);
			...
			foreach($registration['EduRegistrationEvaluation'] as $rev) {
				if($rev['edu_quarter_id'] == $rq['edu_quarter_id'] && 
					$evaluation['EduEvaluation']['id'] == $rev['edu_evaluation_id']) {
						$ev = isset($evaluation_values[$rev['edu_evaluation_value_id']])? $evaluation_values[$rev['edu_evaluation_value_id']]: '-';
						break;
				}
			}
...
$html .= '<table width=100% border=1>
			...
			foreach($registration['EduRegistrationQuarter'] as $rq) {
			$html .= 	
				'<th><center>T' . $cgpacount . '<br/>&nbsp;' . $rq['cgpa'] . '</center></th>';
				$cgpa += $rq['cgpa'];
				$cgpacount++;
			}
$html .= 	
...
$html .= '<table width=100% border=1>
			<tr>
				<td><br/><font size=4px>Your child has been assigned to ____ grade for the next school year</font><br/>&nbsp;</td>
			</tr>
			<tr>
				<td><br/>Administrative Signature<br/>&nbsp;</td>
			</tr>';
$html .= '</table>';

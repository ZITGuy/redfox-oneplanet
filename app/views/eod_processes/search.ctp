...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EodProcess.name', '<?php __('Name'); ?>']
,			['EodProcess.process_date', '<?php __('Process Date'); ?>']
,			['User.name', '<?php __('User'); ?>']
,			['EodProcess.task1_backup_taken', '<?php __('Task1 Backup Taken'); ?>']
,			['EodProcess.task2_portal_updated', '<?php __('Task2 Portal Updated'); ?>']
,			['EodProcess.task3_ftp_sent', '<?php __('Task3 Ftp Sent'); ?>']
,			['EodProcess.backup_type', '<?php __('Backup Type'); ?>']
,			['EodProcess.incremental_count', '<?php __('Incremental Count'); ?>']
,			['EodProcess.backup_incremental_file', '<?php __('Backup Incremental File'); ?>']
,			['EodProcess.backup_full_file', '<?php __('Backup Full File'); ?>']
,			['EodProcess.created', '<?php __('Created'); ?>']
,			['EodProcess.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var eodProcessSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'eodProcesses', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eodProcessSearchWindow.close();
					var conditions = finalGrouping(ParamStore.getRange());
					store_eodProcesses.reload({
						  params: {
								start: 0,
								limit: list_size,
...
var eodProcessSearchWindow = new Ext.Window({
	title: '<?php __('Search EodProcess'); ?>',
	modal: true,
	items: eodProcessSearchForm,
	resizable: false,
	width: 550,
	height: 400,

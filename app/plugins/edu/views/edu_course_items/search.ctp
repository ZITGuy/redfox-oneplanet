...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EduCourseItem.name', '<?php __('Name'); ?>']
,			['EduCourseItem.description', '<?php __('Description'); ?>']
,			['EduCourse.name', '<?php __('EduCourse'); ?>']
,			['EduCourseItem.max_mark', '<?php __('Max Mark'); ?>']
,			['EduCourseItem.created', '<?php __('Created'); ?>']
,			['EduCourseItem.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var eduCourseItemSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduCourseItemSearchWindow.close();
...
var eduCourseItemSearchWindow = new Ext.Window({
	title: '<?php __('Search EduCourseItem'); ?>',
	modal: true,
	items: eduCourseItemSearchForm,
	resizable: false,
	width: 550,
	height: 400,

...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EduExtraPaymentSetting.name', '<?php __('Name'); ?>']
,			['EduClass.name', '<?php __('EduClass'); ?>']
,			['EduExtraPaymentSetting.amount', '<?php __('Amount'); ?>']
...
var eduExtraPaymentSettingSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduExtraPaymentSettingSearchWindow.close();
...
var eduExtraPaymentSettingSearchWindow = new Ext.Window({
	title: '<?php __('Search EduExtraPaymentSetting'); ?>',
	modal: true,
	items: eduExtraPaymentSettingSearchForm,
	resizable: false,
	width: 550,
	height: 400,

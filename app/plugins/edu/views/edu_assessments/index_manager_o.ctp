...
var store_assessments = new Ext.data.GroupingStore({
	...
	proxy: new Ext.data.HttpProxy({
		url: '<?php echo $this->Html->url(array(
            'controller' => 'edu_assessments', 'action' => 'list_data_for_manager')); ?>'
	}),
...
function ReturnAssessment(section_id, course_id) {
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array(
			'controller' => 'edu_assessments', 'action' => 'return_assessment')); ?>/'+section_id+'/'+course_id,
		success: function(response, opts) {
...
function CheckAssessment(section_id, course_id) {
	Ext.Ajax.request({
		url: '<?php echo $this->Html->url(array(
			'controller' => 'edu_assessments', 'action' => 'make_assessments_checked')); ?>/'+section_id+'/'+course_id,
		success: function(response, opts) {
			Ext.Msg.alert('<?php __('Success'); ?>', '<?php __('Assessment checked successfully!'); ?>');
			RefreshAssessmentData();
		},
		failure: function(response, opts) {
			Ext.Msg.alert('<?php __('Error'); ?>',
				'<?php __('Cannot check the selected course assessments. Error code'); ?>: ' + response.status);
		}
	});
}
...
function RefreshAssessmentData() {
	...
	p.getTopToolbar().findById('check-assessment').disable();
}

if(center_panel.find('id', 'assessment-tab') != "") {
	var p = center_panel.findById('assessment-tab');
	...
	var p = center_panel.add({
		...
		tbar: new Ext.Toolbar({
			...
			items: [{
					...
					text: '<?php __('Check'); ?>',
					id: 'check-assessment',
					tooltip:'<?php __('<b>Check Assessments</b><br />Click here to Checked the selected Assessments'); ?>',
					icon: 'img/table_view.png',
					cls: 'x-btn-text-icon',
					disabled: true,
					handler: function(btn) {
						...
						if (sm.hasSelection()) {
							...
							Ext.Msg.show({
								title: "<?php __('Are you sure?'); ?>",
								buttons: Ext.MessageBox.YESNOCANCEL,
								msg: "<?php __('Are you sure want to Checked the selected Assessments?'); ?> ",
								icon: Ext.MessageBox.QUESTION,
								fn: function (btn) {
									if (btn == 'yes') {
										CheckAssessment(sel.data.edu_section_id, sel.data.edu_course_id);
									}
								}
							});
						}
	...
	p.getSelectionModel().on('rowselect', function(sm, rowIdx, r) {
		...
		p.getTopToolbar().findById('check-assessment').enable();

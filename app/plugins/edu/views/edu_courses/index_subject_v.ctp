//<script>
	...
	reader: new Ext.data.JsonReader({
            ...
            fields: [
                ...
                'min_for_pass', 'is_mandatory',
                'description','created','modified'	
            ]
	}),
	proxy: new Ext.data.HttpProxy({
            url: '<?php echo $this->Html->url(array('controller' => 'edu_courses', 'action' => 'list_data_subject', $parent_id)); ?>'	})
    });


    function AddParentEduCourse() {
	Ext.Ajax.request({
            url: '<?php echo $this->Html->url(array('controller' => 'edu_courses', 'action' => 'add_subject', $parent_id)); ?>',
            ...
            failure: function(response, opts) {
                Ext.Msg.alert('<?php __('Error'); ?>', '<?php __('Cannot get the Course add form. Error code'); ?>: ' + response.status);
            }
	});
    }
	...
	Ext.Ajax.request({
            url: '<?php echo $this->Html->url(array('controller' => 'edu_courses', 'action' => 'edit_subject')); ?>/'+id+'/<?php echo $parent_id; ?>',
            success: function(response, opts) {
                var parent_eduCourse_data = response.responseText;
	...
	Ext.Ajax.request({
            ...
            failure: function(response, opts) {
                Ext.Msg.alert('<?php __('Error'); ?>', '<?php __('Cannot get the eduCourse to be deleted. Error code'); ?>: ' + response.status);
            }
	});
    }
	...
	anchor: '100%',
        id: 'eduCourseGrid',
	columns: [
            {header: "<?php __('Grade'); ?>", dataIndex: 'edu_class', sortable: true},
	...
	tbar: new Ext.Toolbar({
            items: [{
                    xtype: 'tbbutton',
                    text: '<?php __('Add'); ?>',
                    tooltip:'<?php __('<b>Add Course</b><br />Click here to create a new Course'); ?>',
                        ...
                        if (sm.hasSelection()){
                            if(sel.length==1){
                                ...
                                Ext.Msg.show({
                                    title: '<?php __('Remove Course'); ?>',
                                    buttons: Ext.MessageBox.YESNOCANCEL,
                                    msg: '<?php __('Remove the selected Course'); ?>?',
                                    icon: Ext.MessageBox.QUESTION,
                                    fn: function(btn){
                                        if (btn == 'yes'){
                                            var sel_ids = '';
                    ...
                    menu : {
                        items: [{
                            ...
                            handler: function(btn) {
                                ...
                                var sel = sm.getSelected();
                                if (sm.hasSelection()){
                                    ViewEduCourseEduAssessments(sel.data.id);
                                };
                            }
                        }, {
                            ...
                            handler: function(btn) {
                                ...
                                var sel = sm.getSelected();
                                if (sm.hasSelection()){
                                    ViewEduCourseEduAssignments(sel.data.id);
                                };
                            }
                        }
                        ]
...
var parentEduCoursesViewWindow = new Ext.Window({
	title: 'Course Under the selected Subject',
	width: 700,
	height:375,
	minWidth: 700,
	minHeight: 400,

...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['AcctFiscalYear.name', '<?php __('Name'); ?>']
,			['AcctFiscalYear.start_date', '<?php __('Start Date'); ?>']
,			['AcctFiscalYear.end_date', '<?php __('End Date'); ?>']
,			['AcctFiscalYear.created', '<?php __('Created'); ?>']
,			['AcctFiscalYear.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var acctFiscalYearSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'acctFiscalYears', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					acctFiscalYearSearchWindow.close();
...
var acctFiscalYearSearchWindow = new Ext.Window({
	title: '<?php __('Search AcctFiscalYear'); ?>',
	modal: true,
	items: acctFiscalYearSearchForm,
	resizable: false,
	width: 550,
	height: 400,

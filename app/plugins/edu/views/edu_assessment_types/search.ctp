...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['AssessmentType.name', '<?php __('Name'); ?>']
		]
	}),
	mode: 'local',
...
var assessmentTypeSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'assessmentTypes', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					assessmentTypeSearchWindow.close();
...
var assessmentTypeSearchWindow = new Ext.Window({
	title: '<?php __('Search AssessmentType'); ?>',
	modal: true,
	items: assessmentTypeSearchForm,
	resizable: false,
	width: 550,
	height: 400,

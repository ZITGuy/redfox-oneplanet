...
,			['AttendanceRecord.date', '<?php __('Date'); ?>']
,			['AttendanceRecord.created', '<?php __('Created'); ?>']
		]
	}),
	mode: 'local',
...
var attendanceRecordSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'attendanceRecords', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					attendanceRecordSearchWindow.close();
...
var attendanceRecordSearchWindow = new Ext.Window({
	title: '<?php __('Search AttendanceRecord'); ?>',
	modal: true,
	items: attendanceRecordSearchForm,
	resizable: false,
	width: 550,
	height: 400,

...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['ReportCategory.name', '<?php __('Name'); ?>']
		]
	}),
	mode: 'local',
...
var reportCategorySearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'reportCategories', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					reportCategorySearchWindow.close();
					var conditions = finalGrouping(ParamStore.getRange());
					store_reportCategories.reload({
						  params: {
								start: 0,
								limit: list_size,
...
var reportCategorySearchWindow = new Ext.Window({
	title: '<?php __('Search ReportCategory'); ?>',
	modal: true,
	items: reportCategorySearchForm,
	resizable: false,
	width: 550,
	height: 400,

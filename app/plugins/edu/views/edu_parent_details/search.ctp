...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EduParentDetail.short_name', '<?php __('Short Name'); ?>']
,			['EduParentDetail.first_name', '<?php __('First Name'); ?>']
,			['EduParentDetail.middle_name', '<?php __('Middle Name'); ?>']
,			['EduParentDetail.last_name', '<?php __('Last Name'); ?>']
,			['EduParentDetail.residence_address', '<?php __('Residence Address'); ?>']
,			['EduParentDetail.nationality', '<?php __('Nationality'); ?>']
,			['EduParentDetail.relationship', '<?php __('Relationship'); ?>']
,			['EduParentDetail.occupation', '<?php __('Occupation'); ?>']
,			['EduParentDetail.academic_qualification', '<?php __('Academic Qualification'); ?>']
,			['EduParentDetail.employment_status', '<?php __('Employment Status'); ?>']
,			['EduParentDetail.employer', '<?php __('Employer'); ?>']
,			['EduParentDetail.work_address', '<?php __('Work Address'); ?>']
,			['EduParentDetail.work_telephone', '<?php __('Work Telephone'); ?>']
,			['EduParentDetail.mobile', '<?php __('Mobile'); ?>']
,			['EduParentDetail.email', '<?php __('Email'); ?>']
,			['EduParentDetail.photo_file', '<?php __('Photo File'); ?>']
,			['EduParentDetail.family_type', '<?php __('Family Type'); ?>']
,			['EduParent.name', '<?php __('EduParent'); ?>']
,			['EduParentDetail.created', '<?php __('Created'); ?>']
,			['EduParentDetail.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var eduParentDetailSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'eduParentDetails', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduParentDetailSearchWindow.close();
					var conditions = finalGrouping(ParamStore.getRange());
					store_eduParentDetails.reload({
						  params: {
								start: 0,
								limit: list_size,
...
var eduParentDetailSearchWindow = new Ext.Window({
	title: '<?php __('Search EduParentDetail'); ?>',
	modal: true,
	items: eduParentDetailSearchForm,
	resizable: false,
	width: 550,
	height: 400,

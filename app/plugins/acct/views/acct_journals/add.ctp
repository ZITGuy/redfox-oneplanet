//<script>
		...
		?>
		var AcctJournalAddForm = new Ext.form.FormPanel({
			baseCls: 'x-plain',
			labelWidth: 100,
			labelAlign: 'right',
		...
		var AcctJournalAddWindow = new Ext.Window({
			title: '<?php __('Add Acct Journal'); ?>',
			width: 400,
			minWidth: 400,
			autoHeight: true,
			layout: 'fit',
			...
			buttonAlign:'right',
			items: AcctJournalAddForm,
			tools: [{
				id: 'refresh',
				qtip: 'Reset',
				handler: function () {
					AcctJournalAddForm.getForm().reset();
				},
				scope: this
			}, {
				id: 'help',
				...
				handler: function () {
					Ext.Msg.show({
						...
						msg: 'This form is used to insert a new Acct Journal.',
				...
				handler: function () {
					if(AcctJournalAddWindow.collapsed)
						AcctJournalAddWindow.expand(true);
					else
						AcctJournalAddWindow.collapse(true);
				}
			}],
			buttons: [  {
				text: '<?php __('Save'); ?>',
				handler: function(btn){
					AcctJournalAddForm.getForm().submit({

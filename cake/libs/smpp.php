...
class SMPP {
     ...
     * @param $login - ESME system_id  smtpuser
     * @param $pass - ESME password
    ...
    function bindReceiver($login, $pass) {
        if ($this->state != "open")
            return false;
        if ($this->debug) {
            echo "Binding receiver...<br/>\n\n";
        }

        $status = $this->_bind($login, $pass, 0x00000001);
        if ($this->debug) {
            echo "Binding status  : $status<br/>\n\n";
        }
        if ($status === 0)
            $this->state = "bind_rx";
        return ($status === 0);
    ...
    function _bind($login, $pass, $command_id) {
        ...
        $pdu = pack(
                ...
                'CCCa' . (strlen($this->address_range) + 1), $login, $pass, $this->system_type, $this->interface_version, $this->addr_ton, $this->addr_npi, $this->address_range);
    ...
    function readPDU_resp($seq_number, $command_id) {
        ...
        if (count($this->pdu_queue) == 0) {
            if ($this->debug) {
                echo "PDU Queue is empty\n\n";
            }
            //return false;
        }
        //check queue
    ...
    function readPDU() {
        //read PDU length
        //echo "I am before socket fread.<br/>\n\n";
        $tmp = @fread($this->socket, 4);
        //echo "I am after socket fread.<br/>\n\n";
        if (!$tmp)
            return false;

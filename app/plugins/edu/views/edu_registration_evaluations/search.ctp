...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EduRegistration.name', '<?php __('Registration'); ?>'],
			['EduEvaluation.name', '<?php __('Evaluation'); ?>'],
			['EduQuarter.name', '<?php __('Quarter'); ?>'],
			['EduEvaluationValue.name', '<?php __('Evaluation Value'); ?>'],
			['EduRegistrationEvaluation.created', '<?php __('Created'); ?>'],
			['EduRegistrationEvaluation.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var eduRegistrationEvaluationSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduRegistrationEvaluationSearchWindow.close();
...
var eduRegistrationEvaluationSearchWindow = new Ext.Window({
	title: '<?php __('Search EduRegistrationEvaluation'); ?>',
	modal: true,
	items: eduRegistrationEvaluationSearchForm,
	resizable: false,
	width: 550,
	height: 400,

//<script>
        ...
        if(sel_edu_course_id !=''){
            Ext.Ajax.request({
				url: '<?php echo $this->Html->url(array(
                    ...
                    ?>?edu_course_id='+sel_edu_course_id+'&edu_section_id='+0,
    ...
    function AddCopyParentAssessment() {
        if (sel_edu_course_id !='') {
            Ext.Ajax.request({
				url: '<?php echo $this->Html->url(array(
                    'controller' => 'edu_assessments', 'action' => 'add_copy_from_previous_term'));
                    ?>?edu_class_id='+sel_edu_class_id,
				success: function(response, opts) {
                    var parent_assessment_data = response.responseText;
			        
                    eval(parent_assessment_data);
			
                    EduAssessmentAddCopyWindow.show();
				},
				failure: function(response, opts) {
                    Ext.Msg.alert('<?php __('Error'); ?>',
                        '<?php __('Cannot get the assessment copy form. Error code'); ?>: ' + response.status);
				}
            });
		} else {
            alert('Please Fill in the required form');
            ...
            <?php
				...
				$options['listeners'] = "{
					...
					'select': function(combo, record, index){
						...
						sel_edu_class_id = combo.getValue();
                        g.getTopToolbar().findById('copy-parent-assessment').enable();
    ...
    var g = new Ext.grid.GridPanel({
        ...
        tbar: new Ext.Toolbar({
            items: [{
                    ...
                    xtype: 'tbbutton',
                    text: '<?php __('Copy'); ?>',
                    id: 'copy-parent-assessment',
                    tooltip:'<?php __('<b>Copy Assessments from Previous Term</b><br />Click here to copy assessment from previous year term 3 of the same class selected.'); ?>',
                    icon: 'img/table_add.png',
                    cls: 'x-btn-text-icon',
                    disabled: true,
                    handler: function(btn) {
                        AddCopyParentAssessment();
                    }
                }, ' ', '-', ' ', {
                    xtype: 'tbbutton',
                    text: '<?php __('Edit'); ?>',
                        ...
                        if (sm.hasSelection()){
                            if(sel.length==1){
                                Ext.Msg.show({
                                    ...
                                    msg: '<?php __('Remove'); ?> '+
                                        sel[0].data.assessment_type+' of '+sel[0].data.max_value+'?',
    ...
    g.getSelectionModel().on('rowselect', function(sm, rowIdx, r) {
	    ...
	    if('<?php echo $user_id; ?>' == record.get('user_id') || '<?php echo $username; ?>' == 'sa') {

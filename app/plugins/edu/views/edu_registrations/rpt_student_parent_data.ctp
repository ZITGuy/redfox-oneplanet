//<script>
	...
	function viewRptStudentParentData() {
		...
		url = "<?php echo $this->Html->url(array('controller' => 'edu_registrations', 'action' => 'rpt_view_student_parent_data')); ?>/" + selected_academic_year_id + '/' + newstr;
		popUpWindow(url, 0, 0, 1200, 1200);
	}
	
	var RptStudentParentDataForm = new Ext.form.FormPanel({
		baseCls: 'x-plain',
		labelWidth: 100,
		labelAlign: 'right',
		url:'',
		...
		items: [
			<?php 
				$options = array('value' => 'Student Parent Data', 'id' => 'data[EduRegistration][report_title]');
	...
	var RptStudentParentDataWindow = new Ext.Window({
		title: '<?php __('Report: Student Parent Data'); ?>',
		width: 400,
		minWidth: 400,
		autoHeight: true,
		layout: 'fit',
		...
		buttonAlign:'right',
		items: RptStudentParentDataForm,
		tools: [{
			id: 'refresh',
			qtip: 'Reset',
			handler: function () {
				RptStudentParentDataForm.getForm().reset();
			},
			scope: this
		}, {
			id: 'help',
			...
			handler: function () {
				Ext.Msg.show({
					...
					msg: 'This form is used to set parameters to the Student Parent Data report.',
			...
			handler: function () {
				if(RptStudentParentDataWindow.collapsed)
					RptStudentParentDataWindow.expand(true);
				else
					RptStudentParentDataWindow.collapse(true);
			}
		}],
		buttons: [{
			text: '<?php __('Show report'); ?>',
			handler: function(btn){
				viewRptStudentParentData();
			}
		}, {
			text: '<?php __('Cancel'); ?>',
			handler: function(btn){
				RptStudentParentDataWindow.close();
			}
		}]
	});
	
	RptStudentParentDataWindow.show();

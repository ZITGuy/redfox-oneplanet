//<script>
	...
	store_assessment_records.on('load', function(records, options){
		//alert(records[0].get('student'));
	}, this );
            ...
            ?>, {
                ...
                listeners:{
                    ...
                    'select': function(combo, record, index) {
                        ...
                        Ext.getCmp('btnUploadExcel').enable();
                    }
                }
            }, {
                xtype: 'combo',
                ...
                listeners:{
                    ...
                    'select': function(combo, record, index) {
						...
						arrayOfStrings = sel_edu_assessment_id.split('-');
						if(arrayOfStrings.length == 2) {
							sel_edu_assessment_id = arrayOfStrings[0];
							if(arrayOfStrings[1] == 'SAVED')
    ...
    var EduAssessmentRecordingWindow = new Ext.Window({
        ...
        buttons: [{
            ...
            handler : function() {
                ...
                Ext.getCmp('btnUploadExcel').disable();
                ...
                if(all_negative) {
					...
					Ext.Ajax.request({
						...
						method: 'POST',
						success: function() {
							Ext.Msg.alert("<?php __('Success'); ?>", "<?php __('Assessment Records created/updated successfully!'); ?>");
                    ...
                    Ext.Msg.alert('<?php __('Error'); ?>',
                        '<?php __('Some values are above the expected, Please correct them, and try again!'); ?>');
            ...
            handler: function(btn){
                var edu_assessment_id = Ext.getCmp('edu_assessment_id');
                if(edu_assessment_id.store.getCount() == 0){
                    Ext.Msg.alert("Oooops!", "<?php __('What is this?'); ?>");
                    return;
                }
				EduAssessmentRecordingTopForm.disable();
				g.disable();
    ...
    function uploadExcel() {
		if (sel_edu_course_id !='' && sel_edu_section_id != '') {
            Ext.Ajax.request({
				url: '<?php echo $this->Html->url(array(
                    'controller' => 'edu_assessments', 'action' => 'upload_excel'));
                    ?>',
				success: function(response, opts) {
                    ...
                    EduAssessmentUploadExcelWindow.show();
				},
				failure: function(response, opts) {
                    Ext.Msg.alert('<?php __('Error'); ?>',
                        '<?php __('Cannot get the assessment upload form. Error code'); ?>: ' + response.status);
				}
            });
		} else {

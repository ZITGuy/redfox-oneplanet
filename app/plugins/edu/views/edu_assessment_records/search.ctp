...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['Student.name', '<?php __('Student'); ?>']
,			['Assessment.name', '<?php __('Assessment'); ?>']
,			['AssessmentRecord.rank', '<?php __('Rank'); ?>']
		]
	}),
	mode: 'local',
...
var assessmentRecordSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					assessmentRecordSearchWindow.close();
...
var assessmentRecordSearchWindow = new Ext.Window({
	title: '<?php __('Search AssessmentRecord'); ?>',
	modal: true,
	items: assessmentRecordSearchForm,
	resizable: false,
	width: 550,
	height: 400,

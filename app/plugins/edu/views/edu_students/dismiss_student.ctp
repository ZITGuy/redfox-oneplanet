//<script>
    ...
    var EduStudentDismissalForm = new Ext.form.FormPanel({
        baseCls: 'x-plain',
        labelWidth: 100,
        labelAlign: 'right',
        url:'<?php echo $this->Html->url(array('controller' => 'edu_students', 'action' => 'dismiss_student')); ?>',
        ...
        items: [{
            ...
            ?>,
            <?php 
                $options_reason = array('fieldLabel' => 'Dismissal Reason', 'xtype' => 'textarea');
                $this->ExtForm->input('reason', $options_reason);
            ?>
        ]
    });
		
    var EduStudentDismissalWindow = new Ext.Window({
        title: '<?php __('Dismiss Student(s)'); ?>',
        width: 700,
        minWidth: 700,
        autoHeight: true,
        layout: 'fit',
        ...
        buttonAlign:'right',
        items: EduStudentDismissalForm,
        tools: [{
            id: 'refresh',
            qtip: 'Reset',
            handler: function () {
                EduStudentDismissalForm.getForm().reset();
            },
            scope: this
        }, {
            id: 'help',
            ...
            handler: function () {
                if(EduStudentDismissalWindow.collapsed)
                        EduStudentDismissalWindow.expand(true);
                else
                        EduStudentDismissalWindow.collapse(true);
            }
        }],
        buttons: [{
                text: '<?php __('Dismiss'); ?>',
                handler: function(btn){
                    EduStudentDismissalForm.getForm().submit({
                        ...
                        success: function(f,a){
                            ...
                            EduStudentDismissalWindow.close();
                        },
    ...
    EduStudentDismissalWindow.show();
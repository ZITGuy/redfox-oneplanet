...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EduClass.name', '<?php __('Name'); ?>'],			
			['EduClass.cvalue', '<?php __('Cvalue'); ?>'],			
			['EduClass.created', '<?php __('Created'); ?>'],			
			['EduClass.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var eduClassSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'eduClasses', 'action' => 'search')); ?>',
...
var eduClassSearchWindow = new Ext.Window({
	title: '<?php __('Search EduClass'); ?>',
	modal: true,
	items: eduClassSearchForm,
	resizable: false,
	width: 550,
	height: 400,

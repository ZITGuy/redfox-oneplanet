//<script>
    var store_eduStudents = new Ext.data.GroupingStore({
        reader: new Ext.data.JsonReader({
            ...
            fields: [
                ...
                'edu_parent', 'status', 'edu_class', 'edu_section'
            ]
        }),
        proxy: new Ext.data.HttpProxy({
            url: "<?php echo $this->Html->url(array('controller' => 'edu_registrations', 'action' => 'list_data_students')); ?>"
        }),
    ...
    foreach ($edu_classes as $item) {
        if ($st) echo ","; ?>['<?php echo $item['EduClass']['id']; ?>', '<?php echo $item['EduClass']['name']; ?>']<?php $st = true;
} ?>]
                        }),
                        displayField: 'name',
                        valueField: 'id',

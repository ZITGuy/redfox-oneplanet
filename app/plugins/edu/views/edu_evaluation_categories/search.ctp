...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EduEvaluationCategory.name', '<?php __('Name'); ?>']
,			['EduEvaluationCategory.list_order', '<?php __('List Order'); ?>']
		]
	}),
	mode: 'local',
...
var eduEvaluationCategorySearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduEvaluationCategorySearchWindow.close();
...
var eduEvaluationCategorySearchWindow = new Ext.Window({
	title: '<?php __('Search EduEvaluationCategory'); ?>',
	modal: true,
	items: eduEvaluationCategorySearchForm,
	resizable: false,
	width: 550,
	height: 400,

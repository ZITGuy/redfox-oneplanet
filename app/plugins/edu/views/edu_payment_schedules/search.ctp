...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EduPaymentSchedule.month', '<?php __('Month'); ?>']
,			['EduClass.name', '<?php __('EduClass'); ?>']
,			['EduPaymentSchedule.amount', '<?php __('Amount'); ?>']
		]
	}),
	mode: 'local',
...
var eduPaymentScheduleSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduPaymentScheduleSearchWindow.close();
...
var eduPaymentScheduleSearchWindow = new Ext.Window({
	title: '<?php __('Search EduPaymentSchedule'); ?>',
	modal: true,
	items: eduPaymentScheduleSearchForm,
	resizable: false,
	width: 550,
	height: 400,

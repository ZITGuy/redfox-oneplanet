...
class MigratorController extends AppController {
    ...
    public function result() {
		$file_name = IMAGES . 'migration_2014_ammends.xlsx';
		$objReader = PHPExcel_IOFactory::createReader('Excel2007');
		$objPHPExcel = $objReader->load($file_name);
		$results = '';
		foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
			echo $worksheet->getTitle() . '<br/>';
			if($worksheet->getTitle() == 'KG2 x') {
			...
			elseif($worksheet->getTitle() == 'Prep2') {
			...
			elseif($worksheet->getTitle() == 'G10 x') {

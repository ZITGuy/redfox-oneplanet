...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EduEvaluationValue.name', '<?php __('Name'); ?>']
,			['EduEvaluationValue.description', '<?php __('Description'); ?>']
		]
	}),
	mode: 'local',
...
var eduEvaluationValueSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduEvaluationValueSearchWindow.close();
...
var eduEvaluationValueSearchWindow = new Ext.Window({
	title: '<?php __('Search Evaluation Value'); ?>',
	modal: true,
	items: eduEvaluationValueSearchForm,
	resizable: false,
	width: 550,
	height: 400,

...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['HelpItem.title', '<?php __('Title'); ?>']
,			['HelpItem.code', '<?php __('Code'); ?>']
,			['HelpItem.content', '<?php __('Content'); ?>']
,			['HelpItemType.name', '<?php __('HelpItemType'); ?>']
,			['HelpItem.list_order', '<?php __('List Order'); ?>']
,			['HelpItem.lft', '<?php __('Lft'); ?>']
,			['HelpItem.rght', '<?php __('Rght'); ?>']
,			['HelpItem.version', '<?php __('Version'); ?>']
,			['HelpItem.created', '<?php __('Created'); ?>']
,			['HelpItem.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var helpItemSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'helpItems', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					helpItemSearchWindow.close();
					var conditions = finalGrouping(ParamStore.getRange());
					store_helpItems.reload({
						  params: {
								start: 0,
								limit: list_size,
...
var helpItemSearchWindow = new Ext.Window({
	title: '<?php __('Search HelpItem'); ?>',
	modal: true,
	items: helpItemSearchForm,
	resizable: false,
	width: 550,
	height: 400,

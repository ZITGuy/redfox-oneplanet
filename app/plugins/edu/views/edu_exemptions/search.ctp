...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			...
			['EduExemption.created', '<?php __('Created'); ?>'],
			['EduExemption.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var eduExemptionSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'edu_exemptions', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduExemptionSearchWindow.close();
					var conditions = finalGrouping(ParamStore.getRange());
					store_eduExemptions.reload({
						  params: {
								start: 0,
								limit: list_size,
...
var eduExemptionSearchWindow = new Ext.Window({
	title: '<?php __('Search Exemption'); ?>',
	modal: true,
	items: eduExemptionSearchForm,
	resizable: false,
	width: 550,
	height: 400,

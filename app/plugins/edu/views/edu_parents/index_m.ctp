...
var store_eduParents = new Ext.data.Store({
	reader: new Ext.data.JsonReader({
		...
		fields: [
			...
			'sms_phone_number', 'portal_record','created','modified'		
		]
	}),
...
function PushParentDataToPortal(id) {
	Ext.Ajax.request({
		url: "<?php echo $this->Html->url(array('controller' => 'edu_parents', 'action' => 'push_parent_data_to_portal')); ?>/"+id,
		success: function(response, opts) {
			var eduParent_data = response.responseText;
		...
		failure: function(response, opts) {
			Ext.Msg.alert("<?php __('Error'); ?>", "<?php __('Cannot get the Parent edit form. Error code'); ?>: " + response);
		}
	});
}
...
function ViewEduParent(id) {
    Ext.Ajax.request({
        ...
        failure: function(response, opts) {
            Ext.Msg.alert("<?php __('Error'); ?>", "<?php __('Cannot get the Parents view form. Error code'); ?>: " + response.status);
        }
    });
}
...
function ViewParentEduStudents(id) {
    Ext.Ajax.request({
        url: "<?php echo $this->Html->url(array('controller' => 'edu_parents', 'action' => 'index_students')); ?>/"+id,
        success: function(response, opts) {
            var parent_eduStudents_data = response.responseText;

            eval(parent_eduStudents_data);

            parentEduParentStudentsViewWindow.show();
        },
        failure: function(response, opts) {
            Ext.Msg.alert("<?php __('Error'); ?><?php __('Cannot get the Parent Students List view. Error code'); ?>: " + response.status);
        }
    });
}
...
function SearchByEduParentName(value){
	var conditions = '\'OR\' => array(\'EduParentDetail.first_name LIKE\' => \'%' + value + '%\', \'EduParentDetail.middle_name LIKE\' => \'%' + value + '%\', \'EduParentDetail.last_name LIKE\' => \'%' + value + '%\')';
...
if(center_panel.find('id', 'eduParent-tab') != "") {
	...
	var p = center_panel.add({
		title: '<font color=#0000aa><?php __('All Parents'); ?><sup>m</sup></font>',
		closable: true,
		loadMask: true,
		stripeRows: true,
		id: 'eduParent-tab',
		...
		tbar: new Ext.Toolbar({
			...
			items: [{
					...
					text: "<?php __('Push'); ?>",
					id: 'push-eduParent',
					tooltip: "<?php __('<b>Push Parent</b><br />Click here to push the selected Parent to Portal'); ?>",
					icon: 'img/table_edit.png',
					cls: 'x-btn-text-icon',
					disabled: true,
					handler: function(btn) {
						...
						var sel = sm.getSelected();
						if (sm.hasSelection()){
							PushParentDataToPortal(sel.data.id);
						};
					}
				}, ' ', '-', ' ', {
					xtype: 'tbbutton',
					...
					tooltip: "<?php __('<b>Students under selected Parent</b><br />Click here to view the Parent Students'); ?>",
					icon: 'img/table_edit.png',
					cls: 'x-btn-text-icon',
					disabled: true,
					handler: function(btn) {
						...
						var sel = sm.getSelected();
						if (sm.hasSelection()){
							ViewParentEduStudents(sel.data.id);
	...
	p.getSelectionModel().on('rowselect', function(sm, rowIdx, r) {
		...
		p.getTopToolbar().findById('push-eduParent').enable();
		p.getTopToolbar().findById('edit-eduParentDetails').enable();
		p.getTopToolbar().findById('edit-eduParentStudents').enable();
		...
		if(this.getSelections().length > 1){
			...
			p.getTopToolbar().findById('push-eduParent').disable();
			p.getTopToolbar().findById('edit-eduParentDetails').disable();
			p.getTopToolbar().findById('edit-eduParentStudents').disable();

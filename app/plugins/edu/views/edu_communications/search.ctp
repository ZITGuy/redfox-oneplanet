...
,			['EduCommunication.post_date', '<?php __('Post Date'); ?>']
,			['EduCommunication.teacher_comment', '<?php __('Teacher Comment'); ?>']
,			['EduCommunication.parent_comment', '<?php __('Parent Comment'); ?>']
,			['User.name', '<?php __('User'); ?>']
,			['EduCommunication.created', '<?php __('Created'); ?>']
,			['EduCommunication.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var eduCommunicationSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'eduCommunications', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduCommunicationSearchWindow.close();
					var conditions = finalGrouping(ParamStore.getRange());
					store_eduCommunications.reload({
						  params: {
								start: 0,
								limit: list_size,
...
var eduCommunicationSearchWindow = new Ext.Window({
	title: '<?php __('Search EduCommunication'); ?>',
	modal: true,
	items: eduCommunicationSearchForm,
	resizable: false,
	width: 550,
	height: 400,

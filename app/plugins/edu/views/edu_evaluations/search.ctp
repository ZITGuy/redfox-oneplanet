...
,			['EduEvaluationArea.name', '<?php __('EduEvaluationArea'); ?>']
,			['EduEvaluation.order_level', '<?php __('Order Level'); ?>']
,			['EduEvaluation.created', '<?php __('Created'); ?>']
,			['EduEvaluation.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var eduEvaluationSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduEvaluationSearchWindow.close();
...
var eduEvaluationSearchWindow = new Ext.Window({
	title: '<?php __('Search EduEvaluation'); ?>',
	modal: true,
	items: eduEvaluationSearchForm,
	resizable: false,
	width: 550,
	height: 400,

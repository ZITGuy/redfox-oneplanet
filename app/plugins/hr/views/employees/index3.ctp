//<script>
   ...
   function ViewOvertime(id){
        Ext.Ajax.request({
            url: '<?php echo $this->Html->url(array('controller' => 'benefits', 'action' => 'index_ot')); ?>/'+id,
            success: function(response, opts) {
                var parent_ot_benefits_data = response.responseText;

                eval(parent_ot_benefits_data);

                parentOTBenefitsViewWindow.show();
            },
            failure: function(response, opts) {
                Ext.Msg.alert('<?php __('Error'); ?>', '<?php __('Cannot get the OT Benefits view form. Error code'); ?>: ' + response.status);
            }
        });  
    }

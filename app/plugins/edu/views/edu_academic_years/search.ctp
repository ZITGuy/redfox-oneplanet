...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['EduAcademicYear.name', '<?php __('Name'); ?>']
,			['EduAcademicYear.start_date', '<?php __('Start Date'); ?>']
,			['EduAcademicYear.end_date', '<?php __('End Date'); ?>']
,			['EduAcademicYear.created', '<?php __('Created'); ?>']
,			['EduAcademicYear.modified', '<?php __('Modified'); ?>']
		]
	}),
	mode: 'local',
...
var eduAcademicYearSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'eduAcademicYears', 'action' => 'search')); ?>',
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					eduAcademicYearSearchWindow.close();
...
var eduAcademicYearSearchWindow = new Ext.Window({
	title: '<?php __('Search EduAcademicYear'); ?>',
	modal: true,
	items: eduAcademicYearSearchForm,
	resizable: false,
	width: 550,
	height: 400,

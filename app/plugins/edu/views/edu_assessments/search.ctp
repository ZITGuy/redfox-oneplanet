...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['AssessmentType.name', '<?php __('AssessmentType'); ?>']
,			['Teacher.name', '<?php __('Teacher'); ?>']
,			['Section.name', '<?php __('Section'); ?>']
,			['Assessment.max_value', '<?php __('Max Value'); ?>']
,			['Assessment.date', '<?php __('Date'); ?>']
,			['Assessment.status', '<?php __('Status'); ?>']
,			['Assessment.detail', '<?php __('Detail'); ?>']
,			['Subject.name', '<?php __('Subject'); ?>']
		]
	}),
	mode: 'local',
...
var assessmentSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
	url: '<?php echo $this->Html->url(array('controller' => 'assessments', 'action' => 'search')); ?>',
...
var assessmentSearchWindow = new Ext.Window({
	title: '<?php __('Search Assessment'); ?>',
	modal: true,
	items: assessmentSearchForm,
	resizable: false,
	width: 550,
	height: 400,

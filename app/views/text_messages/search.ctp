//<script>
	...
	var SearchField = new Ext.form.ComboBox({
		...
		store: new Ext.data.ArrayStore({
			...
			data: [
				['TextMessage.receiver', '<?php __('Receiver'); ?>'],			
				['TextMessage.message', '<?php __('Message'); ?>'],			
				['TextMessage.status', '<?php __('Status'); ?>'],			
				['TextMessage.created', '<?php __('Created'); ?>'],			
				['TextMessage.modified', '<?php __('Modified'); ?>']
			]
		}),
		mode: 'local',
	...
	var textMessageSearchForm = new Ext.form.FormPanel({
		baseCls: 'x-plain',
		labelWidth: 55,
		url: '<?php echo $this->Html->url(array('controller' => 'text_messages', 'action' => 'search')); ?>',
	...
	var textMessageSearchWindow = new Ext.Window({
		title: '<?php __('Search Text Message'); ?>',
		modal: true,

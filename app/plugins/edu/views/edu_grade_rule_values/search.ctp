...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['GradeRuleValue.min', '<?php __('Min'); ?>']
,			['GradeRuleValue.max', '<?php __('Max'); ?>']
,			['GradeRuleValue.code', '<?php __('Code'); ?>']
,			['GradeRule.name', '<?php __('GradeRule'); ?>']
		]
	}),
	mode: 'local',
...
var gradeRuleValueSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					gradeRuleValueSearchWindow.close();
...
var gradeRuleValueSearchWindow = new Ext.Window({
	title: '<?php __('Search GradeRuleValue'); ?>',
	modal: true,
	items: gradeRuleValueSearchForm,
	resizable: false,
	width: 550,
	height: 400,

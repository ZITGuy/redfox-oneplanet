...
$html .= '<table width=100%>
		...
		<tr>
			<td width=98% valign=bottom align=center>
				...
				<br/>
				<h1>' . $registration['EduSection']['EduAcademicYear']['name'] . '</h1>
				<br/>
			</td>
		</tr>
		<tr>
...
if(isset($exemptions)) {
	foreach($exemptions as $exemption) {
		$exempted_courses[] = $exemption['EduExemption']['edu_course_id'];
	}
}
...
foreach ($registration['Rqrs'] as $rqr) {
	if(in_array($rqr['EduCourse']['id'], $exempted_courses)) {
		continue;
	}
	if($rqr['EduSubject'] == null) {
		continue;
	}
...
foreach ($courses as $course) {
	$rqr_count = count($course['Rqrs']);
	if($rqr_count < 3) {
		while($rqr_count < 3) {
			$course['Rqrs'][] = array('EduRegistrationQuarterResult' => array(
				'edu_registration_quarter_id' => 0,
...
foreach ($registration['ExtraEvaluationCategory'] as $category) {
	if (!isset($category['Evaluations'])) { $category['Evaluations'] = array(); }
	foreach ($registration['Evaluation'] as $evaluation) {
		if ($evaluation['EduEvaluationArea']['edu_evaluation_category_id'] == $category['EduEvaluationCategory']['id']) {
			$category['Evaluations'][] = $evaluation;
...
$registration['ExtraEvaluationCategory'] = $extra_categories;
unset($registration['Evaluation']);

$extra_rec_count = 1;
foreach ($registration['ExtraEvaluationCategory'] as $cat) {
	$extra_rec_count += count($cat['Evaluations']) + 1;
}
...
// extra-curricular for classes above 8 (i.e., 9 -> Year 10, 10 -> Year 11, 11 -> Year 12, 12 -> Year 13)
if ($registration['EduClass']['cvalue'] > 8) {
	$html .= '<table width=100% border=1 bgcolor=#cccccc>
			...
			<tr>
				<td>EP = Excellent Performance</td><td>A</td><td>4.0</td>
			</tr>
			<tr>
				<td>AE = Above Expectation</td> <td>B</td> <td>3.0</td>
			</tr>
			<tr>
				<td>ME = Meets Expectation</td> <td>C</td> <td>2.0</td>
			</tr>
			<tr>
				<td>BE = Below Expectation</td> <td>D</td> <td>1.0</td>
			</tr>
			<tr>
				<td>U =  Unsatisfactory</td> <td>F</td> <td>0</td>
			</tr>
		</table>';
	...
	$count_of_extras = 0;
		foreach ($registration['ExtraEvaluationCategory'] as $category) {
			$evcount=1;
			$count_of_extras = (count($category['Evaluations']));
			$html .= '<tr bgcolor=#cccccc>
						<th width=40%>&nbsp;</th>
						<th width=12%><center>T1</center></th>
						<th width=12%><center>T2</center></th>
						<th width=12%><center>T3</center></th>
						<th width=24%><center>Comments</center></th>
					</tr>';
			foreach ($category['Evaluations'] as $evaluation) {
				$html .=
						'<tr>
				...
				foreach ($registration['EduRegistrationQuarter'] as $rq) {
					...
					$tcgpas[$rq['edu_quarter_id']] = !isset($tcgpas[$rq['edu_quarter_id']])? 0: $tcgpas[$rq['edu_quarter_id']];
					$tcgpas[$rq['edu_quarter_id']] += ($ev == 'A'? 4: ($ev == 'B'? 3: ($ev == 'C'? 2: ($ev == 'D'? 1: 0))));
					$count_of_extras -= ($ev == 'NA'? 1: 0);
					$possible_values = array('A', 'B', 'C', 'D', 'F', 'NA');
					$count_of_extras -= (in_array($ev, $possible_values)? 0: 1);
					$html .= '<td><center>' . $ev . '</center></td>';
				}
				$html .=  ($evcount == 1? '<td rowspan=4>T1</td>':
						($evcount == 5? '<td rowspan=4>T2</td>': ($evcount == 9? '<td rowspan=2>T3</td>': ''))) .
	...
	// end of the extra-curricular activities table
	...
	if($count_of_extras <= 0) {
		$count_of_extras = 1;
	}
	foreach ($registration['EduRegistrationQuarter'] as $rq) {
		$html .= '<td>' . (isset($tcgpas[$rq['edu_quarter_id']]) && $tcgpas[$rq['edu_quarter_id']] != 0? number_format($tcgpas[$rq['edu_quarter_id']]/$count_of_extras, 2, '.', ',') : '-') . '</td>';
	}
	$html .=
			'<td>&nbsp;</td>
		</tr>
